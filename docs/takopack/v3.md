...
下表のように `takos` には拡張機能開発向けの API 群が集約されています。詳細なメソッドは[APIと権限](#apiと権限)を参照してください。
...
- **getURL**: `takos.getURL(): string[]`
  - 現在の URL パスを配列で取得します。
- **pushURL**: `takos.pushURL(segment: string, options?: { showBar?: boolean }): void`
  - `segment` を末尾に追加して遷移します。
- **setURL**: `takos.setURL(segments: string[], options?: { showBar?: boolean }): void`
  - URL 全体を配列で指定して遷移します。
- **changeURL**: `takos.changeURL(listener: (e: { url: string[] }) => void): () => void`
