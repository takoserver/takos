...
PubSub.subscribe("client", async (_subpubType: string, message: string) => {
  ...
  if (data.type === "migrateRequest") {
    ...
    for (const user of users) {
      ...
      for (const sessionid of sessionids) {
        const ws = sessions.get(sessionid);
        if (ws) {
          const { migrateid, requesterSessionid } = JSON.parse(
            message,
          );
  ...
  if (data.type === "callAccept") {
    ...
    for (const user of users) {
      ...
      for (const sessionid of sessionids) {
        const ws = sessions.get(sessionid);
        if (ws) {
          const { sessionid: targetSessionid } = JSON.parse(message);
