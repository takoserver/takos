...
export interface TakosCallEventMap {
    ...
    transportCreated: { roomId: string, peerId: string, transportId: string, direction: string, transportInfo: TransportInfo };
    transportClosed: { roomId: string, peerId: string, transportId: string };
    producerCreated: { roomId: string, peerId: string, producerId: string, transportId: string, kind: string, producerInfo: ProducerInfo };
    producerClosed: { roomId: string, peerId: string, producerId: string };
    consumerCreated: { roomId: string, peerId: string, consumerId: string, producerId: string, transportId: string, consumerInfo: ConsumerInfo };
...
export class TakosCallClient {
    ...
    private isValidEventName(event: string): event is TakosCallEventName {
        const validEvents: TakosCallEventName[] = [
            "connected", "disconnected", "reconnecting", "error", "message",
            "roomCreated", "roomClosed", "peerAdded", "peerClosed",
            "transportCreated", "transportClosed", "producerCreated", "producerClosed",
            "consumerCreated", "consumerClosed"
        ];
        return validEvents.includes(event as TakosCallEventName);
    }

    // 型安全なemitメソッド
    private emitSafely<K extends TakosCallEventName>(event: K, data: TakosCallEventMap[K]): void {
    ...
    // イベント処理メソッドを型付きに更新
    public on<K extends TakosCallEventName>(event: K, callback: EventCallback<TakosCallEventMap[K]>): void {
    ...
    public off<K extends TakosCallEventName>(event: K, callback?: EventCallback<TakosCallEventMap[K]>): void {
    ...
    private emit<K extends TakosCallEventName>(event: K, data: TakosCallEventMap[K]): void {
        if (!this.eventListeners.has(event)) return;
