...
async function runDevCommandInDir(dir: string): Promise<Deno.CommandStatus> {
  ...
  try {
    ...
    if (status.success) {
      ...
      console.error(
        `[INFO] 'deno task dev' in ./${dir} exited with code ${status.code}.`,
      );
    }
    return status;
  } catch (error) {
    if (error instanceof Error) {
      console.error(
        `[INFO] Failed to start 'deno task dev' in ./${dir}: ${error.message}`,
      );
    } else {
      console.error(
        `[INFO] Failed to start 'deno task dev' in ./${dir}: ${String(error)}`,
      );
    }
  ...
  try {
    ...
    results.forEach((status, index) => {
      if (!status.success) {
        console.warn(
          `[INFO] Process for ./${
            targetDirectories[index]
          } exited with error code: ${status.code}`,
        );
      }
    });
  } catch (error) {
    if (error instanceof Error) {
      console.error(
        "[INFO] An error occurred while managing dev tasks:",
        error.message,
      );
    } else {
      console.error(
