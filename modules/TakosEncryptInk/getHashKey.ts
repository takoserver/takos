async function areKeysEqual(key1: CryptoKey, key2: CryptoKey) {
  async function getHashKey(key: CryptoKey): Promise<ArrayBuffer> {
  ...
  // 2つの鍵のハッシュを取得
  const hash1 = await getHashKey(key1);
  const hash2 = await getHashKey(key2);
