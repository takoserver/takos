...
const test = async () => {
  ...
  const takoIdentityKeys = await (async () => {
    ...
    for (let i = 0; i < 5; i++) {
      keys.push(
        await ((await createIdentityKeyAndAccountKey(tako)).identityKey),
      );
    }
    return keys;
  })();
  const ikaIdentityKeys = await (async () => {
    ...
    for (let i = 0; i < 5; i++) {
      keys.push(
        await ((await createIdentityKeyAndAccountKey(ika)).identityKey),
      );
    }
    return keys;
  })();
  ...
  const messagesChain = await (async (): Promise<messageBlock[]> => {
    ...
    for (let i = 0; i < 5; i++) {
      messages.push(
        await createMessageBlock(
          await encryptDataRoomKey(roomKey, JSON.stringify(message)),
          takoIdentityKeys[i],
          messages[i - 1],
        ),
      );
      messages.push(
        await createMessageBlock(
          await encryptDataRoomKey(roomKey2, JSON.stringify(message)),
          ikaIdentityKeys[i],
          messages[i],
        ),
      );
  ...
  const messages = await verifyAndDecryptMessageChain(
    messagesChain.map((messageBlock, index) => {
      return {
