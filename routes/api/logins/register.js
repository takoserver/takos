      ...
      if(requirments == "register") {
        ...
        if(ismail) {
          if(!ismailduplication) {
              ...
              await tempUsers.create({mail: email, key: key, checkCode: checkCode})
              sendMail(email,"takoserverです次のtokenでメールアドレスを認証してください",`token: ${checkCode}`)
              return new Response(JSON.stringify({status: true, mailToken: key}), {
                headers: { "Content-Type": "application/json"},
                status : 200,
              });
              } catch (error) {
        ...
        const variablesToCheck = [userName,nickName,password,age,mailToken,isagreement];
        ...
        if (!/^[ぁ-んァ-ンa-zA-Z0-9]{1,18}$/.test(nickName)) {
            return new Response(JSON.stringify({"status": false, error: "nickName"}), {
                headers: { "Content-Type": "application/json" },
                status : 403
            });
        }
        ...
        if(!tempUserInfo.checked) {
            return new Response(JSON.stringify({status: false,error: "checked"}), {
                headers: { "Content-Type": "application/json"},
                status : 403
            });
        }
        ...
        if(await isUserDuplication(userName)) {
            return new Response(JSON.stringify({status: false,error: "userdup"}), {
                headers: { "Content-Type": "application/json"},
                status : 403
            });
        }
        ...
        if(await isMailDuplication(mail)) {
            return new Response(JSON.stringify({status: false,error: "maildup"}), {
                headers: { "Content-Type": "application/json"},
                status : 403
            });
        }
        ...
        //ユーザーを登録
        await users.create({userName,nickName, mail: mail, password: hashHex, salt: salt, age: age});
        //tempUsersから削除
        ...
        if(mailToken === undefined || mailToken === "" ||  mailToken === null || checkCode === undefined || checkCode === "" ||  checkCode === null) {
          return new Response(JSON.stringify({"status": false,error: "input"}), {
            headers: { "Content-Type": "application/json"},
            status: 403,
          });
        ...
        if(tempUserInfo.checkCode != checkCode) {
          return new Response(JSON.stringify({"status": false,error: "token"}), {
            headers: { "Content-Type": "application/json"},
            status: 403,
          });
        }
        await tempUsers.updateOne({checkCode: checkCode},{$set: {checked: true}})
        return new Response(JSON.stringify({status: true}), {
          headers: { "Content-Type": "application/json"},
          status: 200,
        });
