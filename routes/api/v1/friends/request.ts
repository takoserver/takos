...
export const handler = {
  async POST(req: Request, ctx: any) {
    ...
    if (data.type === "AddFriendKey") {
      ...
      const existingRequest = await requestAddFriend.findOne({ userID: addFriendUserInfo._id.toString() });

      if (!existingRequest) {
        await requestAddFriend.create({ userID: addFriendUserInfo._id.toString(), Applicant: [{ userID: userid }] });
      } else {
      ...
      const friendRequest = await requestAddFriend.findOne({ userName: friendName });
      if (!friendRequest) {
        return new Response(JSON.stringify({ status: "error" }), {
          headers: { "Content-Type": "application/json" },
          status: 403,
