...
export const handler = {
  async GET(req: Request, ctx: Context): Promise<Response> {
    ...
    if (isRequestList === "true") {
      const FriendInfo: User | null = await users.findOne({ userName: friendName });
      const AddfriendInfo: RequestAddFriend | null = await reqestAddFriend.findOne({
      ...
      try {
        const filePath = `./files/userIcons/${mailSpilit(FriendInfo.uuid)}.webp`;
    ...
    if (isuseAddFriendKey === "true") {
      ...
      const user: User | null = await users.findOne({ addFriendKey: addFriendKey });
      if (user == null) {
      ...
      try {
        const filePath = `./files/userIcons/${mailSpilit(user.uuid)}.webp`;
        const result = await Deno.readFile(filePath);
        return new Response(result, {
          headers: { "Content-Type": "image/webp" },
          status: 200,
    ...
    if (friendDomain === env["serverDomain"]) {
      ...
      try {
        const filePath = `./files/userIcons/${splitUserName(friendid)?.name}.webp`;
        const result = await Deno.readFile(filePath);
        return new Response(result, {
          headers: { "Content-Type": "image/webp" },
          status: 200,
      ...
      const resUserUUID = await fetch(`http://${friendDomain}/api/v1/server/users/${friendName}/uuid`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
...
async function fetchWithSizeLimit(input: RequestInfo, limit: number): Promise<Response> {
