    ...
    async GET(req, ctx) {
        ...
        switch (requirments) {
            case "getFriendIcon": {
                ...
                return new Response(JSON.stringify({icon: result.friendIcon,status: true}), {
                    headers: { "Content-Type": "application/json" },
                });
            }
            case "getMyIcon":
...
async function addFriendIcon(friendKey) {
    const friendInfo = await users.findOne({ addFriendKey: friendKey },{userName:1});
...
async function getFriendIcon(friendName, userName) {
    const friendInfo = await friends.findOne({ userName: friendName },{friends:1});
    if (friendInfo === null || friendInfo === undefined) {
        return { status: false };
    }
    const friend = friendInfo.friends.find((friend) => friend.userName === userName);
