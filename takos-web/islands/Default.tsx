...
export default function setDefaultState({ state }: { state: AppStateType }) {
  ...
  async function decryptIdentityAndAccountKeys(
    deviceKey: deviceKey,
    keys: any[],
  ) {
    ...
    if (updates.allowedKey.length !== 0) {
      for (const key of updates.allowedKey) {
        ...
        const signKey = state.IdentityKeyAndAccountKeys.value.find(
          (key: IdentityKeyAndAccountKeysState) =>
            key.hashHex === signKeyhashHex,
        );
        if (!signKey) {
          continue;
        }
    ...
    state.ws.value.onmessage = async (event) => {
      ...
      switch (data.type) {
        ...
        case "messageFriend":
          {
            ...
            if (state.friendid.value === friendId) {
              const talkData = await fetch(
                `/takos/v2/client/talk/data/friend`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                  ...
                  body: JSON.stringify({
                    ...
                    ignoreKeys: (() => {
                      ...
                      const hashHexArray = roomKeys.map((key) =>
                        key.roomKey.hashHex
                      );
                      return hashHexArray;
                    })(),
                    ...
                    ignoreIdentityKeys: (() => {
                      ...
                      const hashHexArray = identityKeys.map((key) =>
                        key.hashHex
                      );
                      return hashHexArray;
                    })(),
        ...
        case "keyShareAccept":
          {
            ...
            if (!migrateDataSignKeyRes) {
              ...
              migrateDataSignKeyPublic.value =
                migrateDataSignKeyRes.migrateDataSignKeyPublic;
              const hash = fnv1a(
        ...
        case "keyShareData": {
          ...
          const identityAndAccountKeys =
            migrateDataJson.identityAndAccountKeysData;
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const { identityKey, accountKey } =
                        await createIdentityKeyAndAccountKey(
                          masterKey,
                        );
                      const deviceKey = await createDeviceKey(masterKey);
                      const keyShareKey = await createKeyShareKey(masterKey);
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (resJson.status) {
                        ...
                        console.log(await db.get("deviceKey", "deviceKey"));
                        const res = await fetch(
                          "/takos/v2/client/profile/keys/geted",
                          {
                            method: "POST",
                            headers: {
                              "Content-Type": "application/json",
              ...
              <form
                ...
                onSubmit={async (e) => {
                  ...
                  const identityAndAccountKeysData = JSON.stringify(
                    decryptedIdentityAndAccountKeys.filter((key) =>
                      key !== null
                    ),
                  );
