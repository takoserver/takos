...
export default async function getKeys(deviceKeyPrivate: deviceKeyPrivate) {
  ...
  const encryptedIdentityKey = keys.find((key) =>
    key.keyType === "identityKey"
  );
  ...
  const identityKey = await decryptDataDeviceKey(
    deviceKey,
    JSON.parse(encryptedIdentityKey.encryptedKey),
  );
  const masterKey = await decryptDataDeviceKey(
    deviceKey,
    JSON.parse(encryptedMasterKey.encryptedKey),
  );
  const accountKey = await decryptDataDeviceKey(
    deviceKey,
    JSON.parse(encryptedAccountKey.encryptedKey),
  );
