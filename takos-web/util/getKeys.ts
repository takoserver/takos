...
export default async function getKeys(
  ...
  try {
    ...
    if(!deviceKeyPub || !encryptedMasterKey || !encryptedKeyShareKeys) {
      throw new Error("deviceKeyPub, encryptedMasterKey, encryptedKeyShareKeys is not found");
    }
    ...
    const masterKeyString = await decryptDataDeviceKey(deviceKey, encryptedMasterKey.masterKey);
    const keyShareKeysString = await Promise.all(encryptedKeyShareKeys.map(async (keyShareKey) => {
    ...
    const keyShareKeys: KeyShareKey[] = keyShareKeysString.map((keyShareKey) => keyShareKey ? JSON.parse(keyShareKey) : null);
    const identityKeyAndAccountKeys = await Promise.all(keys.map(async (key) => {
      ...
      const identityKeyString = await decryptAndVerifyDataWithKeyShareKey(keyShareKey, encryptedIdentityKey.key, masterKey.public);
      const accountKeyString = await decryptAndVerifyDataWithKeyShareKey(keyShareKey, encryptedAccountKey.key, masterKey.public);
    ...
    if(!masterKey || !deviceKey || !keyShareKeys || !identityKeyAndAccountKeys) {
      throw new Error("masterKey, deviceKey, keyShareKeys, identityKeyAndAccountKeys is not found");
    }
    return {
      masterKey,
      deviceKey,
