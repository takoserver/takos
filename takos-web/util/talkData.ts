  ...
  if (roomType === "friend") {
    ...
    for (const key in talkData.identityKeys) {
      ...
      for (const identityKey of identityKeys) {
        ...
        for (const key of allowKeys) {
          if (
            key.keyHash === await generateKeyHashHexJWK(masterKey.masterKey)
          ) {
            allowKey = key;
          }
        }
        if (!allowKey) {
          if (userId === state.userId.value) {
            ...
            const hashHexMyMasterKey = await generateKeyHashHexJWK(
              myMasterKey.public,
            );
            const hashHexThisMasterKey = await generateKeyHashHexJWK(
              masterKey.masterKey,
            );
    ...
    for (const message of talkData.messages) {
      const roomKey = resultRoomKeyArray.find(
        (key) =>
          key &&
          String(message.message.value.data.encryptedKeyHashHex) ===
            key.hashHex,
      )?.key;
      if (!roomKey) {
        continue;
      ...
      if (
        new Date(message.timestamp).getTime() -
            new Date(messageObj.timestamp).getTime() > 1000 * 60
      ) {
        continue;
      }
      talkDataTemp.push({
        ...
        verify: verifyResult,
        identityKeyHashHex: await generateKeyHashHexJWK(
    ...
    for (const [_index, message] of talkDataTemp.entries()) {
      if (
        messageMasterKeyTimestamp.find((key) =>
          key.hashHex === message.masterKeyHashHex
        )
      ) {
        continue;
      }
    ...
    for (const message of talkDataTemp) {
      ...
      const index = messageMasterKeyTimestamp.findIndex((key) =>
        key.hashHex === hashHex
      );
      ...
      if (
        new Date(masterKeyInfo.timestamp).getTime() >
