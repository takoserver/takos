...
export default function RegisterForm({ text,token,sitekey }: { text: string, token: string; sitekey: string;}) {
    ...
    const handleCheckCodeChange = (event: h.JSX.TargetedEvent<HTMLInputElement>) => {
      setcheckCode(event.currentTarget.value)
    }
    const handleNickNameChange = (event: h.JSX.TargetedEvent<HTMLInputElement>) => {
      setNickName(event.currentTarget.value)
  }
  const handleUserNameChange = (event: h.JSX.TargetedEvent<HTMLInputElement>) => {
      setUserName(event.currentTarget.value);
  };
  const handlePasswordChange = (event: h.JSX.TargetedEvent<HTMLInputElement>) => {
      setPassword(event.currentTarget.value);
  };
  const handleAgeChange = (event: any) => {
      setAge(event.currentTarget.value);
  };
  const handleAgreementChange = (event: h.JSX.TargetedEvent<HTMLInputElement>) => {
    ...
    const mailCkeckHandleSubmit = async (event: JSX.TargetedEvent<HTMLFormElement, Event>) => {
      event.preventDefault()
      const data = {
    ...
    const registerHandleSubmit = async (event: JSX.TargetedEvent<HTMLFormElement, Event>) => {
      event.preventDefault()
      console.log()
      const data = {
      ...
      if(data.userName === undefined || data.nickName === undefined || data.password === undefined || data.age === undefined || data.userName === "" || data.nickName === "" || data.password === "" || data.age === "") {
        setShowError(true)
          ...
          <div className="bg-[#000000] lg:w-1/3 w-[90%] h-[90%] lg:h-4/5 mx-auto my-[15%] lg:my-[5%] p-5 rounded-xl">
            ...
            <div class="w-4/5 mx-auto my-0">
              ...
              <div class="text-center text-sm">
                ...
                {showForm == "closed" && (
                 <>
                  <EmailForm onSubmit={handleSubmit} showError={showEmailError} errorMessages={emailErrorMessages} onChange={handleEmailChange} value={email} />
                 </>
                )
                }
                {showForm == "checkMail" && (<>
                  <CheckEmailForm onChange={handleCheckCodeChange} value={checkCode} onSubmit={mailCkeckHandleSubmit} showError={showCheckCodeError} errorMessages={checkCodeErrorMessages} />
                </>)}
                ...
                {showForm == "finished" && (
                  <div class="text-white text-3xl">本登録が完了しました。閉じてログインしてください</div>
                )
                }
              </div>
            </div>
...
function EmailForm({onChange,value,onSubmit, showError, errorMessages}: {onChange: any,value: any,onSubmit: (event: h.JSX.TargetedEvent<HTMLFormElement, Event>) => void, showError: boolean, errorMessages: string}) {

  return (<>
  <form onSubmit={onSubmit} class="max-w-sm mx-auto">
 <Input placeholder="tako@example.com" onChange={onChange} value={value} title="メールアドレス" type="email" showError={showError} errorMessage={errorMessages} />
...
function CheckEmailForm({onChange,value,onSubmit, showError, errorMessages}: {onChange: any,value: any,onSubmit: (event: h.JSX.TargetedEvent<HTMLFormElement, Event>) => void, showError: boolean, errorMessages: string}) {
  return (<>
    <form onSubmit={onSubmit} class="max-w-sm mx-auto">
    <p class="text-white hover:underline font-medium text-xl mb-10">確認コードを送信しました。認証してください</p>
      <Input placeholder="xxxxxx" onChange={onChange} value={value} title="確認コード" type="number" showError={showError} errorMessage={errorMessages} />
        <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
          送信
          </button>
      </form>
...
function MainRegisterForm({userNameOnChange,nickNameOnChnage,passwordOnChange,ageOnChange,agreementOnChange,userName,nickName,password,age,isagreement,onSubmit,showUsernameError,showNickNameError,showPasswordError,showAgeError,showIsAgreementError,userNameErrorMessage,nicknameErrorMessage,passwordErrorMessage,ageErrorMessage,isAgreementErrorMessage}:
  {userNameOnChange: any,nickNameOnChnage: any,passwordOnChange: any,ageOnChange: any,agreementOnChange: any,userName: string,nickName: string,password: string,age: any,isagreement: boolean,onSubmit: (event: h.JSX.TargetedEvent<HTMLFormElement, Event>) => void,showUsernameError: boolean,showNickNameError: boolean,showPasswordError: boolean,showAgeError: boolean,showIsAgreementError: boolean,errorMessages: string,userNameErrorMessage: string,nicknameErrorMessage: string,passwordErrorMessage: string,ageErrorMessage: string,isAgreementErrorMessage: string}) {
  return (
  <>
  <form onSubmit={onSubmit} class="max-w-sm mx-auto">
 <Input placeholder="tako@example.com" onChange={userNameOnChange} value={userName} title="ユーザーネーム" type="text" showError={showUsernameError} errorMessage={userNameErrorMessage} />
  <Input placeholder="tako" onChange={nickNameOnChnage} value={nickName} title="ニックネーム" type="text" showError={showNickNameError} errorMessage={nicknameErrorMessage} />
  <Input placeholder="tako" onChange={passwordOnChange} value={password} title="パスワード" type="password" showError={showPasswordError} errorMessage={passwordErrorMessage} />
  <Input placeholder="tako" onChange={ageOnChange} value={age} title="年齢" type="number" showError={showAgeError} errorMessage={ageErrorMessage} />
  <div class="flex items-start mb-5">
    <div class="flex items-center h-5">
      <input
      onChange={agreementOnChange}
      checked={isagreement}
      type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800" required />
      {showIsAgreementError && (
    <div class="text-red-500 text-xs">{isAgreementErrorMessage}</div>
  )}
    </div>
    <label for="remember" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">利用規約に同意</label>
  </div>
  <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
    送信
    </button>
