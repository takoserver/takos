  ...
  if (!Keys2) {
    ...
    Keys2 = Keys2.map((k) => {
      return {
        timestamp: k.timestamp,
        hashHex: k.hashHex,
        encryptedIdentityKey: k.encryptedIdentityKey.find((
          i: { sessionid: string },
        ) => i.sessionid === sessionid),
        encryptedAccountKey: k.encryptedAccountKey.find((
          i: { sessionid: string },
        ) => i.sessionid === sessionid),
      };
    }).filter((k) =>
      k.encryptedIdentityKey !== undefined &&
      k.encryptedAccountKey !== undefined
    );
  }
  return c.json({
    status: true,
