...
app.get("/", async (c) => {
  try {
    ...
    return c.json({ success: false, error: "拡張機能一覧の取得に失敗しました" }, 500);
  }
});
...
app.post("/install", async (c) => {
  try {
    ...
    if (!file) {
      return c.json({ success: false, error: "パッケージファイルが必要です" }, 400);
    }
    ...
    if (!manifestText) {
      return c.json({ success: false, error: "manifest.json が見つかりません" }, 400);
    }

    let manifest;
    try {
      manifest = JSON.parse(manifestText);    } catch (_e) {
      return c.json({ success: false, error: "manifest.json が無効です" }, 400);
    }

    // 必須フィールドの検証
    if (!manifest.identifier || !manifest.name || !manifest.version) {
      return c.json({ success: false, error: "manifest.json に必須フィールドが不足しています" }, 400);
    }

    // server.js の取得
    const serverJs = extractedFiles["takos/server.js"];
    if (!serverJs) {
      return c.json({ success: false, error: "server.js が見つかりません" }, 400);
    }    // client.js の取得
    ...
    // 既存の拡張機能をチェック
    const existingExtension = await Extention.findOne({ id: manifest.identifier });
    ...
    return c.json({ success: false, error: "拡張機能のインストールに失敗しました" }, 500);
  }
});
...
app.delete("/:id", async (c) => {
  try {
    ...
    return c.json({ success: true, message: "拡張機能をアンインストールしました" });
  } catch (error) {
    console.error("Uninstall extension error:", error);
    return c.json({ success: false, error: "拡張機能のアンインストールに失敗しました" }, 500);
  }
});
...
app.post("/:id/toggle", async (c) => {
  try {
    ...
    return c.json({ 
      success: true, 
      message: enabled ? "拡張機能を有効にしました" : "拡張機能を無効にしました" 
    });
  } catch (error) {
    console.error("Toggle extension error:", error);
    return c.json({ success: false, error: "拡張機能の切り替えに失敗しました" }, 500);
  }
});
...
app.get("/:id", async (c) => {
  try {
    ...
    return c.json({ success: false, error: "拡張機能の取得に失敗しました" }, 500);
  }
});
...
app.get("/marketplace/search", (c) => {
  try {
    ...
    return c.json({ success: false, error: "マーケットプレイス検索に失敗しました" }, 500);
  }
});

// TakoPackage ファイルの解析
async function extractTakoPackage(packageBuffer: Uint8Array): Promise<Record<string, string>> {
  try {
