...
import { ActivityPubActivity, VerificationResult } from "./types/activitypub.ts"; // 型をインポート
...
export interface ExtensionContext {
  ...
  events: {
    publish: (eventName: string, payload: unknown) => Promise<[number, Record<string, unknown>]>;
...
class ExtensionHookManager {
  ...
  async loadExtension(extensionId: string): Promise<void> {
    try {
      ...
      const manifest: ExtensionManifest = extensionModel.manifest as ExtensionManifest;
      ...
      // サーバーモジュールを評価
      const serverModule = await this.evaluateServerModule(extensionModel.serverjs, manifest);
  ...
  private getHookFromExtension(extensionIdentifier: string, hookName: string): ((...args: unknown[]) => unknown) | undefined {
    const extension = this.loadedExtensions.get(extensionIdentifier);
    if (extension && extension.serverModule && typeof extension.serverModule[hookName] === 'function') {
      return extension.serverModule[hookName] as (...args: unknown[]) => unknown;
    }
    return undefined;
  }

  // ActivityPub受信フック処理
  async processIncomingActivity(activity: ActivityPubActivity): Promise<HookResult> {
    const relevantExtensions = Array.from(this.loadedExtensions.values())
      .filter(ext => this.isActivityRelevant(ext.manifest, activity))
      .sort((a, b) => (b.manifest.activityPub?.objects?.[0]?.hooks?.priority || 0) - 
    ...
    for (const extensionData of relevantExtensions) {
      if (extensionData.manifest.activityPub?.objects?.[0]?.hooks?.canAccept) {
        try {
          const hookResult = await this.callExtensionHook(extensionData.manifest.identifier, "canAccept", activity);
          // hookResultが { accepted: boolean } 形式であることを期待
          if (hookResult && typeof hookResult === 'object' && 'accepted' in hookResult && hookResult.accepted === false) {
            console.log(`Activity rejected by canAccept hook in ${extensionData.manifest.identifier}`);
            return { accepted: false, processedActivity: activity };
          }
        } catch (error) {
          console.error(`canAccept hook failed for ${extensionData.manifest.identifier}:`, error);
    ...
    if (serialMode) {
      // 順次実行
      for (const extensionData of relevantExtensions) {
        if (extensionData.manifest.activityPub?.objects?.[0]?.hooks?.onReceive) {
          try {
            const result = await this.callExtensionHook(extensionData.manifest.identifier, "onReceive", processedActivity);
            if (result && typeof result === 'object' && 'type' in result && 'id' in result) { // ActivityPubActivity型かチェック
              processedActivity = result as ActivityPubActivity;
            }
          } catch (error) {
            console.error(`onReceive hook (serial) failed for ${extensionData.manifest.identifier}:`, error);
          }
        }
      }
    } else {
      ...
      const hookPromises = relevantExtensions
        .filter(ext => ext.manifest.activityPub?.objects?.[0]?.hooks?.onReceive)
        .map(extensionData => 
          this.callExtensionHook(extensionData.manifest.identifier, "onReceive", processedActivity)
            .then(result => {
              if (result && typeof result === 'object' && 'type' in result && 'id' in result) { // ActivityPubActivity型かチェック
            ...
            .catch(error => {
              console.error(`onReceive hook (parallel) failed for ${extensionData.manifest.identifier}:`, error);
              return null; 
            })
        );
  ...
  // ActivityPub送信フック処理
  async processOutgoingActivity(activity: ActivityPubActivity): Promise<HookResult> { // async を追加
    // 送信時のフック処理（必要に応じて実装）
    ...
    for (const extensionData of relevantExtensions) {
      ...
      const onSendHook = this.getHookFromExtension(extensionData.manifest.identifier, "onSendActivity"); 
      if (onSendHook) {
        try {
          const result = await onSendHook(extensionData.context, processedActivity);
          if (result && typeof result === 'object' && 'type' in result && 'id' in result) {
            processedActivity = result as ActivityPubActivity;
          }
        } catch (error) {
          console.error(`onSendActivity hook failed for ${extensionData.manifest.identifier}:`, error);
        }
      }
    }
    return { accepted: true, processedActivity };
  ...
  private isActivityRelevant(manifest: ExtensionManifest, activity: ActivityPubActivity): boolean {
  ...
  ): Promise<ActivityPubActivity | VerificationResult | { accepted: boolean; processedActivity?: unknown } | null> {
    const extensionData = this.loadedExtensions.get(extensionIdentifier);
    if (!extensionData) {
      console.warn(`Extension ${extensionIdentifier} not loaded for hook call.`);
    ...
    if (hook && typeof hook === "function") {
      ...
      return Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error(`Hook timeout for ${hookName} in ${extensionIdentifier}`)), 5000)
        ),
      ]) as Promise<ActivityPubActivity | VerificationResult | { accepted: boolean; processedActivity?: unknown } | null>;
    }
    return Promise.resolve(null);
  }  // サーバーモジュールの評価（セキュリティ強化版）
  private evaluateServerModule(serverCode: string, manifest: ExtensionManifest): Record<string, unknown> {
    // セキュリティポリシーを適用
    const secureCode = this.applySecurityPolicy(serverCode, manifest);
      // 制限されたグローバルコンテキスト
    const allowedGlobals = {
      console: {
        log: (...args: unknown[]) => console.log(`[${manifest.identifier}]`, ...args),
        error: (...args: unknown[]) => console.error(`[${manifest.identifier}]`, ...args),
        warn: (...args: unknown[]) => console.warn(`[${manifest.identifier}]`, ...args),
        info: (...args: unknown[]) => console.info(`[${manifest.identifier}]`, ...args),
      },
    ...
    const moduleContext = {
      ...
      process: { 
        env: manifest.permissions.includes("deno:env") ? Deno.env.toObject() : {},
    ...
    try {
      ...
      console.error(`Code evaluation failed for ${manifest.identifier}:`, error);
      throw error; // エラーを再スロー
    }
  }

  // セキュリティポリシーを適用
  private applySecurityPolicy(code: string, _manifest: ExtensionManifest): string {
    // 危険なパターンをチェック
    ...
    for (const pattern of dangerousPatterns) {
      if (pattern.test(code)) {
        throw new Error(`Security policy violation: Dangerous pattern detected (${pattern.source})`);
      }
    }

    // コードサイズ制限（1MB）
    if (code.length > 1024 * 1024) {
      throw new Error("Security policy violation: Code size limit exceeded (1MB)");
    }
...
// 拡張機能コンテキストの作成
  private createExtensionContext(manifest: ExtensionManifest): ExtensionContext {
    const context: ExtensionContext = {
      ...
      permissions: manifest.permissions,      kv: {
      ...
      activitypub: {        send: async (userId: string, activity: Record<string, unknown>) => {
          // ActivityPub送信ロジック
          // 権限チェック: manifest.permissions.includes("activitypub:send")
          console.log(`[${manifest.identifier}] AP Send for ${userId}:`, activity);
          await Promise.resolve(); // lint回避のためのダミーawait
        },
        ...
        list: async (userId?: string) => {
          ...
          update: async (userId: string, key: string, value: string) => {
            console.log(`[${manifest.identifier}] AP Actor Update: ${userId}, ${key}=${value}`);
            await Promise.resolve(); // lint回避のためのダミーawait
          },
          delete: async (userId: string, key: string) => {
            console.log(`[${manifest.identifier}] AP Actor Delete: ${userId}, ${key}`);
            await Promise.resolve(); // lint回避のためのダミーawait
          },
        },
        follow: async (followerId: string, followeeId: string) => {
          console.log(`[${manifest.identifier}] AP Follow: ${followerId} -> ${followeeId}`);
          await Promise.resolve(); // lint回避のためのダミーawait
        },
        unfollow: async (followerId: string, followeeId: string) => {
          console.log(`[${manifest.identifier}] AP Unfollow: ${followerId} -> ${followeeId}`);
          await Promise.resolve(); // lint回避のためのダミーawait
        },
        ...
        listFollowing: async (actorId: string) => {
          ...
          create: async (localName: string, profile: Record<string, unknown>) => {
            console.log(`[${manifest.identifier}] Plugin Actor Create: ${localName}`, profile);
            await Promise.resolve(); // lint回避のためのダミーawait
          ...
          update: async (iri: string, partial: Record<string, unknown>) => {
            console.log(`[${manifest.identifier}] Plugin Actor Update: ${iri}`, partial);
            await Promise.resolve(); // lint回避のためのダミーawait
          },
        ...
        write: async (path: string, data: string | Uint8Array, _options?: { cacheTTL?: number }) => {
        ...
        list: async (prefix?: string) => {
          console.log(`[${manifest.identifier}] Asset List with prefix: ${prefix}`);
          await Promise.resolve(); // lint回避のためのダミーawait
          return [];
        },
      },      events: {
        publish: async (eventName: string, payload: unknown): Promise<[number, Record<string, unknown>]> => {
          // イベント発行ロジック
          // 権限チェック: manifest.permissions.includes("events:publish")
          console.log(`[${manifest.identifier}] Event Publish: ${eventName}`, payload);
        ...
        publishToClient: async (eventName: string, payload: unknown) => {
          console.log(`[${manifest.identifier}] Event PublishToClient: ${eventName}`, payload);
          await Promise.resolve(); // lint回避のためのダミーawait
        },
        subscribe: (eventName: string, handler: (payload: unknown) => void): (() => void) => {
          // イベント購読ロジック
        ...
        if (!manifest.permissions.includes("fetch:net")) {
          throw new Error(`[${manifest.identifier}] Permission denied: fetch:net`);
        }
  ...
  async processClientEvent(
    ...
    const eventHandler = this.getHookFromExtension(extensionId, eventHandlerName);
    
    if (typeof eventHandler !== "function") {
      console.warn(`Event handler ${eventHandlerName} not found or not a function in extension ${extensionId}.`);
      throw new Error(`Event handler ${eventHandlerName} not found in extension ${extensionId}`);
    }
    ...
    try {
      ...
      console.error(`Error processing client event ${eventName} in extension ${extensionId}:`, error);
      throw error; 
    }
  }
}
