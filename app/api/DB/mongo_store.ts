...
export function createMongoDataStore(env: Record<string, string>): DataStore {
  ...
  return {
    ...
    dms: {
      save: (from, to, type, content, attachments, url, mediaType, key, iv, preview) =>
        impl.saveDMMessage(from, to, type, content, attachments, url, mediaType, key, iv, preview),
    ...
    host: {
      ...
      findInstanceByHostAndOwner: (h, o) => impl.findInstanceByHostAndOwner(h, o),
