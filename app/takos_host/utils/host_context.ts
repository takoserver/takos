...
// Optional components are not present in this workspace; provide safe fallbacks
const takosEnv: Record<string, string> = {};
const _createRootActivityPubApp = (_env: Record<string, string>) => null as unknown as Hono;
const _createServiceActorApp = (_env: Record<string, string>) => null as unknown as Hono;
const createConsumerApp = (
  _onDrop: (host: string) => void,
  _opts: { rootDomain: string; freeLimit: number; reservedSubdomains: string[] },
) => {
  const app = new Hono();
  app.all("*", () => new Response("Not Found", { status: 404 }));
  return app;
};
const FCM_KEYS: string[] = [];
...
export interface HostContext {
  ...
  rootActivityPubApp: Hono | null;
  serviceActorApp: Hono | null;
  isDev: boolean;
}
...
export async function initHostContext(): Promise<HostContext> {
  ...
  const rootActivityPubApp = rootDomain ? null : null;
  const serviceActorApp = rootDomain ? null : null;
