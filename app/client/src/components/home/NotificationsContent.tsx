          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <Show when={notifications.loading}>
                <div class="animate-spin rounded-full h-4 w-4 border-2 border-teal-400 border-t-transparent"></div>
              </Show>
            </div>
            <div class="flex items-center space-x-3">
              <button
          ...
          <div class="p-6 bg-rose-500/10 border-l-4 border-rose-500">
            <div class="flex items-center space-x-3">
              ...
              <div>
                ...
                <p class="text-rose-300 text-sm">通知の読み込みに失敗しました。再度お試しください。</p>
              </div>
            </div>
          </div>
        </Show>
            ...
            fallback={
              <div class="p-8 text-center">
                <svg class="w-16 h-16 text-gray-600 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M15 17h5l-5 5v-5zM11 19H6a2 2 0 01-2-2V7a2 2 0 012-2h5m5 0v6h6m-6-6L16 9l-5-5z" />
                </svg>
                <p class="text-gray-400 text-lg">通知はありません</p>
                <p class="text-gray-500 text-sm mt-2">新しい通知が届くとここに表示されます</p>
              </div>
            }
          >
                ...
                return (
                  <div 
                    class={`group relative transition-all duration-300 ${
                      isDeleting ? 'opacity-50 scale-95' : 'hover:bg-gray-800/20'
                    } ${!n.read ? 'bg-teal-500/5' : ''}`}
                  > 
                    <div class={`p-6 border-l-4 ${c.border} ${!n.read ? 'border-l-teal-400' : ''}`}>
                      <div class="flex items-start space-x-4">
                        <div class={`w-10 h-10 ${c.bg} rounded-full flex items-center justify-center flex-shrink-0 relative`}>
                          <svg class={`w-5 h-5 ${c.text}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            {c.icon}
                          </svg>
                          <Show when={!n.read}>
                            <div class="absolute -top-1 -right-1 w-3 h-3 bg-teal-400 rounded-full"></div>
                          </Show>
                        </div>
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center justify-between mb-2">
                            <h4 class={`text-lg font-semibold ${!n.read ? 'text-gray-50' : 'text-gray-100'}`}>
                              {n.title}
                            </h4>
                          ...
                          <p class={`text-sm leading-relaxed ${!n.read ? 'text-gray-200' : 'text-gray-300'}`}>
                            {n.message}
                          </p>
        ...
        <div class="p-6 border-t border-gray-800/50">
          <div class="flex items-center justify-between">
            <div class="text-xs text-gray-500">
              未読: {notifications()?.filter(n => !n.read).length || 0}件 / 
