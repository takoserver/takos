...
fn random_group_id() -> GroupId { 
    GroupId::from_slice(&OpenMlsRustCrypto::default().random_vec(16).unwrap()) 
}
...
pub fn generate_key_package() -> Result<KeyPackageResult, JsValue> {
    ...
    let signer = SignatureKeyPair::new(CS.signature_scheme())
...
pub fn create_group() -> Result<CreateGroupResult, JsValue> {
    ...
    // Export group info
    let group_info_bytes = export_group_info_internal(&group, provider, &signer)
...
pub fn decrypt(handle: u32, message_b64: &str) -> Result<DecryptResult, JsValue> {
    ...
    let protocol_message = ProtocolMessage::tls_deserialize(&mut data.as_slice())
