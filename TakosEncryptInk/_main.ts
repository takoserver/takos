...
export async function createIdentityKeyAndAccountKey(
  ...
  const accountHashHex = await crypto.subtle.digest(
    "SHA-256",
  ...
  const identityKeySign = sign(masterKey, idenKeys.publicKey)
...
export function isValidAccountKey(
  ...
  return verify(
    identityKey,
    new Uint8Array(base64ToArrayBuffer(accountKey.key)),
    accountKey.sign,
  )
}
...
export function isValidDeviceKey(
  ...
  return verify(
    masterKey,
    new Uint8Array(base64ToArrayBuffer(deviceKey.key)),
    deviceKey.sign,
  )
}
...
export function isValidRoomKey(
  ...
  return verify(
    identityKey,
    new Uint8Array(base64ToArrayBuffer(roomKey.key)),
    roomKey.sign,
  ) &&
    verify(
      identityKey,
  ...
  return verify(
    ...
    verify(
      ...
      new TextEncoder().encode(
        keyShareKey.timestamp + keyShareKey.keyExpiration,
      ),
...
export function signDataWithMigrateDataSignKey(
  ...
  const key = new Uint8Array(
    base64ToArrayBuffer(migrateDataSignKey.private.key),
  )
...
export async function encryptMessage(
  ...
  if (
    ...
    roomKeyExpiration.getTime() - new Date(roomKey.timestamp).getTime() >
      365 * 24 * 60 * 60 * 1000
  ) {
