...
import type { AccountKeyPrivateObject, AccountKeyPublicObject } from "../../types/keys.ts"
...
export async function DecryptDataAccountKey(
  ...
  const encryptedDataObject: EncryptedDataAccountKeyObject = JSON.parse(encryptedData)
  if (encryptedDataObject.type !== "accountKey") throw new Error("Invalid type")
