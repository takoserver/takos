...
import type { IdentityKeyPrivateObject, roomKeyObject } from "../../types/keys.ts"
...
export async function generateRoomKey(
  ...
  for (const key of keys) {
    ...
    if (!verifyDataMasterKey(key.identityKey.public, key.masterKey, key.identityKey.sign)) {
      throw new Error("Invalid Sign")
    }
    if (!verifyDataIdentityKey(key.accountKey.public, key.identityKey.public, key.accountKey.sign)) {
      throw new Error("Invalid Sign")
    }
    const idenKeyKey: IdentityKeyPublicObject = JSON.parse(key.identityKey.public)
    ...
    for (const latestIdentityKeyTimestamp of latestIdentityKeyTimestamps) {
      if (latestIdentityKeyTimestamp.userId === key.userId) {
        if (new Date(latestIdentityKeyTimestamp.timestamp) > new Date(idenKeyKey.timestamp)) {
          throw new Error("Invalid Identity Key")
        }
        if (new Date(latestIdentityKeyTimestamp.timestamp) < new Date(idenKeyKey.timestamp)) {
    ...
    for (const creditMasterKey of creditMasterKeys) {
      if (creditMasterKey.userId === key.userId) {
        if (creditMasterKey.hash !== masterKeyHash) {
          if (new Date(creditMasterKey.timestamp) > new Date(masterKey.timestamp)) {
            throw new Error("Invalid Master Key")
          }
  ...
  for (const key of keys) {
    const encryptedRoomKey = await EncryptDataAccountKey(roomKey, key.accountKey.public)
