...
import type { migrateKey } from "../types/migrateKey.ts"
...
import { encryptDataMlKems } from "./encryptData.ts"
import { arrayBufferToHex } from "../utils/hex.ts"
export async function createMasterKey(): Promise<MasterKey> {
  ...
  const hashHexString = arrayBufferToHex(hashHex)
  return {
    public: {
      key: publicKeyString,
      keyType: "masterPub",
...
export async function createIdentityKeyAndAccountKey(
  ...
  const idenHashHex = arrayBufferToHex(idenHash)
...
export async function createRoomKey(
  keys: [
    MasterKeyPub,
    { key: IdentityKeyPub; sign: Sign },
    { key: AccountKeyPub; sign: Sign },
    string,
  ][],
  ...
  const roomKeyHashHex = arrayBufferToHex(roomKeyHash)
  const timeStamp = new Date().toISOString()
  const signature = sign(
    identityKey,
    concatenateUint8Arrays([
      new Uint8Array(roomKeyExport),
      new TextEncoder().encode(timeStamp),
      new TextEncoder().encode(JSON.stringify(masterKeysHashHex)),
    ]),
  )
  ...
  for (const key of keys) {
    //Encrypt
    const encryptedData = await encryptDataMlKems(
      JSON.stringify(RoomKeyResult),
      key[2].key,
    )
...
export async function createKeyShareKey(
  ...
  const KeyShareKeyResult: KeyShareKey = {
    ...
    version: 1,
    hashHex: arrayBufferToHex(hash),
  }
  const keyShareKeySign = sign(
    identityKey,
    concatenateUint8Arrays([
      keyShareKey.publicKey,
      new TextEncoder().encode(timestamp),
    ]),
  )
...
export async function createKeyShareSignKey(
  identityKey: IdentityKey,
): Promise<[KeyShareSignKey, Sign]> {
  const keyShareSignKey = ml_kem768.keygen()
  const keyShareSignKeyPublicString = arrayBufferToBase64(
    keyShareSignKey.publicKey,
  )
  const keyShareSignKeyPrivateString = arrayBufferToBase64(
    keyShareSignKey.secretKey,
  )
  const timestamp = new Date().toISOString()
  ...
  const keyShareSignKeySign = sign(
    identityKey,
    concatenateUint8Arrays([
      keyShareSignKey.publicKey,
      new TextEncoder().encode(timestamp),
...
export async function createDeviceKey(): Promise<deviceKey> {
  const deviceKeyRaw = await crypto.subtle.exportKey(
    "raw",
...
export async function createMigrateDataSignKey(): Promise<migrateDataSignKey> {
  ...
  const migrateDataSignKeyPublicString = arrayBufferToBase64(
    migrateDataSignKeyRaw.publicKey,
  )
  const migrateDataSignKeyPrivateString = arrayBufferToBase64(
    migrateDataSignKeyRaw.secretKey,
  )
