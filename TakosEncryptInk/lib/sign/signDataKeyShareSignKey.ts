...
} from "../isValid.ts/keyShareKey.ts";
import { isValidSign } from "../isValid.ts/sign.ts";

export async function signDataKeyShareKey(data: string, key: {
  public: string;
  private: string;
}): Promise<string> {
  const masterKeyPublic: keyShareSignKeyPublicObject = JSON.parse(key.public);
  const masterKeyPrivate: keyShareSignKeyPrivateObject = JSON.parse(
    key.private,
  );
...
export function verifyDataKeyShareKey(
  data: string,
  key: string,
  sign: string,
): boolean {
