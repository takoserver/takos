...
export function isValidRoomKey(key: string): boolean {
  ...
  if (keyObject.timestamp !== new Date(keyObject.timestamp).toISOString()) return false
  if (keyObject.type !== "roomKey") return false
  for (const hash in keyObject.masterKeysHashHex) {
    const hashRaw = new Uint8Array(base64ToArrayBuffer(keyObject.masterKeysHashHex[hash]))
