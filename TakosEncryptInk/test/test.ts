...
async function createUserKeys() {
  ...
  const identityAndAccountKeys = await generateIdentityKeyAndAccountKey(masterkeys)
  return {
    masterkeys,
...
const idenPrivate = new Uint8Array(
  base64ToArrayBuffer(
    (JSON.parse(aliceKeys.identityAndAccountKeys.identityKey.private) as IdentityKeyPrivateObject).key,
  ),
)
