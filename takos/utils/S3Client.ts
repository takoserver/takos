...
console.log(env["MINIO_ENDPOINT"], env["AWS_ACCESS_KEY_ID"], env["AWS_SECRET_ACCESS_KEY"], env["BUCKET_NAME"]);
...
async function downloadFile(key: string): Promise<string> {
    return new Promise((resolve, reject) => {
        minioClient.getObject(env["BUCKET_NAME"] as string, key, (err: Error | null, dataStream: any) => {
            if (err) {
                reject(err);
                return;
            }
