import { generateDeviceKey, isValidIdentityKeyPublic, keyHash, verifyMasterKey } from "@takos/takos-encrypt-ink";
...
app.post("identityKey", zValidator(
    "json",
    z.object({
    ...
    await IdentityKey.updateOne({ userName: user.userName, sessionid: session.sessionid }, { 
        updateTime: Date.now(),
    })
...
app.post("accountKey", zValidator(
    "json",
    z.object({
        accountKey: z.string(),
    ...
    const { accountKey, accountKeySign, encryptedAccountKey } = c.req.valid("json");
    if(!verifyMasterKey(user.masterKey, accountKeySign, accountKey)){
        return c.json({ message: "Invalid account key" }, 400);
    }
    const sessionUUIDs = encryptedAccountKey.map(([sessionUUID]) => sessionUUID);
    const serverSessionUUIDs = (await Session.find({ userName: user.userName })).map((session) => [session.sessionUUID, session.sessionid]);
    //sessionUUIDsに被りがなく、serverSessionUUIDsに全て含まれているか
    if(sessionUUIDs.length !== encryptedAccountKey.length || !sessionUUIDs.every((sessionUUID) => serverSessionUUIDs.some(([serverSessionUUID]) => serverSessionUUID === sessionUUID))){
    ...
    for (const [sessionUUID, encryptedAccountKeyValue] of encryptedAccountKey) {
        await shareAccountKey.create({
            userName: user.userName,
              ...
              sessionid: serverSessionUUIDs.find(([serverSessionUUID]) => serverSessionUUID === sessionUUID)?.[1] ?? null,
        });
    }
    return c.json({ message: "success" });
})
