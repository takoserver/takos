...
app.post("/", zValidator(
  ...
  const { accountKey, accountKeySign, encryptedAccountKeys } = c.req.valid("json");
  if (!verifyMasterKey(user.masterKey, accountKeySign, accountKey)) {
    return c.json({ message: "Invalid account key" }, 400);
  }
  const sessionUUIDs = encryptedAccountKeys.map(([sessionUUID]) => sessionUUID);
